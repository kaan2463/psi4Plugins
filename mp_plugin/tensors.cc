/////////////////////////////////////
// Copyright (c) 2019 Ugur Bozkaya //
/////////////////////////////////////

// Latest revision on July 5, 2019.
#ifdef _MSC_VER
#include <process.h>
...
void Tensor1d::gemv(bool transa, int m, int n, const SharedTensor2d &a, const SharedTensor2d &b, double alpha,
    ...
    // gemv: C(Q) = \sum_{mn} A(mn,Q) b(mn)
    if (transa) {
        if (dim1_ != a->dim2() || a->dim1() != b->dim1() * b->dim2()) {
            outfile->Printf("\tTensor2d::gemv dimensions are NOT consistent!\n");
            throw PSIEXCEPTION("Tensor2d::gemv dimensions are NOT consistent!");
        }
    }
    // gemv: C(Q) = \sum_{mn} A(Q,mn) b(mn)
    else {
        if (dim1_ != a->dim1() || a->dim2() != b->dim1() * b->dim2()) {
            outfile->Printf("\tTensor2d::gemv dimensions are NOT consistent!\n");
            throw PSIEXCEPTION("Tensor2d::gemv dimensions are NOT consistent!");
        }
    }
...
void Tensor2d::contract(bool transa, bool transb, int m, int n, int k, const SharedTensor2d &a, const SharedTensor2d &b,
    ...
    // Check C(m,n): C(m,n) = \sum(k) op(A)(m,k) op(B)(k,n)
    if (m * n != dim1_ * dim2_) {
        outfile->Printf("\tTensor2d::contract the m*n value is NOT consistent with dimensions of the Tensor C!\n");
        throw PSIEXCEPTION("Tensor2d::contract the m*n value is NOT consistent with dimensions of the Tensor C!");
    }

    // Check A(m,k)
    if (m * k != a->dim1() * a->dim2()) {
        outfile->Printf("\tTensor2d::contract the m*k value is NOT consistent with dimensions of the Tensor A!\n");
        throw PSIEXCEPTION("Tensor2d::contract the m*k value is NOT consistent with dimensions of the Tensor A!");
    }

    // Check B(k,n)
    if (n * k != b->dim1() * b->dim2()) {
        outfile->Printf("\tTensor2d::contract the n*k value is NOT consistent with dimensions of the Tensor B!\n");
        throw PSIEXCEPTION("Tensor2d::contract the n*k value is NOT consistent with dimensions of the Tensor B!");
    }
        ...
        for (int k = 1; k <= m; ++k) {
            for (int j = 10 * num_frame_counter + 1; j < 10 * num_frame_counter + 12; j++) {
                if (j == 10 * num_frame_counter + 1) {
                    ...
                    printer->Printf(" %15.10f", a[k - 1][j - 2]);
                }
            }
            printer->Printf("\n");
        }
